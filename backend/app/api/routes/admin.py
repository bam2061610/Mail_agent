...
def get_backups(
    ...
    _log_admin_action(db, current_user, "diagnostics_checked", {"target": "backups", "count": len(backups)})
    return backups
...
def create_backup_route(
    ...
    try:
        ...
        _log_admin_action(db, current_user, "backup_failed", {"error": str(exc)})
...
def get_backups_status(
    ...
    _log_admin_action(db, current_user, "diagnostics_checked", {"target": "backups_status"})
    return response
...
def get_admin_health(
    ...
    _log_admin_action(db, current_user, "diagnostics_checked", {"target": "health", "test_mailboxes": test_mailboxes})
...
def get_admin_diagnostics(
    ...
    _log_admin_action(db, current_user, "diagnostics_checked", {"target": "diagnostics", "test_mailboxes": test_mailboxes})
    return AdminHealthResponse(**diagnostics)
...
def get_admin_jobs(
    ...
    _log_admin_action(db, current_user, "diagnostics_checked", {"target": "jobs"})
...
def get_admin_mailboxes_status(
    ...
    if failed:
        _log_admin_action(db, current_user, "mailbox_health_check_failed", {"failed_mailboxes": failed})
    _log_admin_action(db, current_user, "diagnostics_checked", {"target": "mailboxes_status", "test_connection": test_connection})
    return statuses


def _log_admin_action(db: Session, user: User, action_type: str, payload: dict) -> None:
    db.add(
        ActionLog(
            user_id=user.id,
